...
class NFA(OFA):
    ...
    def product(self, other):
        ...
        def _sN(a, s):
            """Gets the state index of named state s in automaton a or None if DNE"""
            try:
                j = a.stateIndex(s)
            except DFAstateUnknown:
                return None
        ...
        def _kS(a, j):
            """Returns the transitions in a from state index j
        ...
        def _dealT(srcI, dest):
            """Creates a state if needed, then adds a transition to it

            :param srcI: source state
            :param dest: destination state"""
            if not (dest in done or dest in notDone): # === dest not in done AND dest not in notDone
