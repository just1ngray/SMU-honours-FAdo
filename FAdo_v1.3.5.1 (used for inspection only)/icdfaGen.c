...
void randomInit(icdfagen_t gen, int states, int symbols, int t, unsigned long seed){
  struct timeval foo;
  struct timezone bar;
  ...
  if (seed) {
    gmp_randseed_ui(RND_STATE(gen), seed);
    srand(seed);
  } else {
...
int IRAND(int max, int bias, unsigned int *st){
	int foo;
	
/*	foo = RAND(max+bias);*/
    foo = stfull_rand(max+bias, st);
...
void initializeICDFA(icdfagen_t icdfaGen){
  ...
  memset(ICDFA(icdfa), ICDFAGEN_BOTTOM(icdfaGen), ICDFA_SIZE(icdfa) * sizeof(state_t));
...
extern icdfagen_t icdfaGenInit(int states, int symbols, int incomplete, int ibias, int random, unsigned long seed){
  ...
  *icdfa = icdfaInit(states, symbols, incomplete);
  ...
  if (random)
    randomInit(icdfaGen, states, symbols, incomplete, seed);
  else
...
extern icdfa_t icdfaGenRandom(icdfagen_t gen, int incomplete){
  ...
  if (!incomplete){
    for(i = 1; i < ICDFA_NSTATES(aut); i++){
      ...
      FLAGS(gen)[i-1] = f;
      for(j=t1; j<f; j++){
				*pt++ = stfull_rand(i, &(gen->rndst));
      }
      *pt++ = i;
      t1 = f+1;
    }
    for(j = t1; j < ICDFA_NSYMBOLS(aut)*ICDFA_NSTATES(aut); j++)
      *pt++ = stfull_rand(ICDFA_NSTATES(aut), &(gen->rndst));
  } 
    ...
    for(i = 1; i < ICDFA_NSTATES(aut); i++){
      ...
      FLAGS(gen)[i-1] = f;
      for(j=t1; j<f; j++){
				*pt++ = IRAND(i, ICDFAGEN_IBIAS(gen), &(gen->rndst));
      }
      *pt++ = i;
      t1 = f+1;
    }
    for(j = t1; j < ICDFA_NSYMBOLS(aut)*ICDFA_NSTATES(aut); j++)
      *pt++ = IRAND(ICDFA_NSTATES(aut), ICDFAGEN_IBIAS(gen), &(gen->rndst));
  }
...
extern int stfull_rand(int max, unsigned int *rndstate){
  if (*rndstate == 0)
    ...
    srand(*rndstate);
    foo = rand();
    *rndstate = (unsigned int) rand();
    return((int) ((max) * (foo / (RAND_MAX + 1.0))));
    }
  }
